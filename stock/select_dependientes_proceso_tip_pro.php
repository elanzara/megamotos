...
if(validaSelect($selectDestino) && validaOpcion($opcionSeleccionada))
{
	...
	//conectar();
        ...
        Conectarse();
	if ($selectDestino == 'productos')
	{
            $consulta=mysql_query("SELECT p.pro_id
                        ,concat(IFNULL(p.pro_descripcion,' '),' ',IFNULL(p.pro_med_diametro,' ')) pro_descripcion
                                   FROM `productos` p
                                   WHERE p.pro_estado = 0
                                   and p.tip_id = '$opcionSeleccionada'
                                   order by p.pro_descripcion") or die(mysql_error());
 	}
