...
$select = "(select ma.mar_descripcion from marcas ma where ma.mar_id = p.mar_id)
, (select mo.mod_descripcion from modelos mo where mo.mod_id = p.mod_id)
, p.pro_med_diametro
, p.pro_med_ancho
, p.pro_distribucion
...
////$x->setOrderBy("4, 5, 6, 2, 3, 9");
$x->setOrderBy("1, 2, 3, 7");
// Allows filters
$x->allowFilters();
...
//$x->setColumnHeader('pro_descripcion', 'DESCRIPCION');
...
$x->hideColumn('(select tr.tr_descripcion from tipo_rango tr where tr.tr_id = p.tr_id)');
$x->hideColumn('(select v.prv_descripcion from proveedores v where v.prv_id = p.prv_id)');
...
<div id="page"> 
 ...
 <!-- End HEADER -->
  <!--Start CENTRAL -->
 <div id="central">
  <h1>.:Monitor de Stock - LLANTAS ORIGINALES:. </h1>
<!--    <table>
        <tr>
            <td>Sucursal:</td>
            <td>
