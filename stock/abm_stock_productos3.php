...
$select = "(select tp.tip_descripcion from tipo_productos tp where tp.tip_id = p.tip_id) 
...
$where .= " and p.tip_id not in (2, 4, 9, 1)";
//(select ifnull(sum(cast(m.cantidad as signed)*(case m.tim_id when 1 then 1 else (-1) end)),0)
//            from movimientos_stock m
//            where m.estado=0
//            and m.pro_id = p.pro_id
...
"(select tp.tip_descripcion from tipo_productos tp where tp.tip_id = p.tip_id)"
, "(select ma.mar_descripcion from marcas ma where ma.mar_id = p.mar_id)"
, "(select mo.mod_descripcion from modelos mo where mo.mod_id = p.mod_id)"
, "pro_descripcion"
, "pro_nueva"
...
$x->setColumnHeader('(select tp.tip_descripcion from tipo_productos tp where tp.tip_id = p.tip_id)', 'TIPO PRODUCTO');
...
$x->setColumnHeader('cantidad_Caseros','CANT. DEPOSITO');
...
<head>
<title>MEGALLANTASMOTOS - Admin</title>
...
<div id="page"> 
 ...
 <!-- End HEADER -->
  <!--Start CENTRAL -->
 <div id="central">
  <h1>.:Monitor de Stock General:. </h1>
<!--    <table>
        <tr>
            <td>Sucursal:</td>
            <td>
