...
//, (select s.suc_descripcion from sucursales s where s.suc_id = su.suc_id) as sucu
...
//$where .= " and p.tip_id in (2, 3) and p.pro_clasificacion = 'R' ";
...
$where = "p.pro_estado = 0 and p.pro_controla_stock = 'S'";
$where .= " and (
(select m.stm_cantidad
            from stock_mensual m
            where m.pro_id = p.pro_id
            and m.suc_id = 1
            and m.stm_fecha= (select max(m1.stm_fecha) 
                                from stock_mensual m1
                                where m1.pro_id = m.pro_id
                                and m1.suc_id = m.suc_id)
            ) > 0
or
(select m.stm_cantidad
            from stock_mensual m
...
////$x->setOrderBy("4, 5, 1, 7, 8, 9");
$x->setOrderBy("1, 2, 3, 4");
// Allows filters
$x->allowFilters();
...
//$x->setColumnHeader('sucu', 'SUCURSAL');
...
//$x->setColumnHeader('pro_descripcion', 'DISENO');
...
//$x->setColumnHeader('pro_terminacion','TERMINACION');
...
$x->hideColumn('(select tr.tr_descripcion from tipo_rango tr where tr.tr_id = p.tr_id)');
//$x->hideColumn('(select ma.mar_descripcion from marcas ma where ma.mar_id = p.mar_id)');
//$x->hideColumn('(select mo.mod_descripcion from modelos mo where mo.mod_id = p.mod_id)');
...
<div id="page"> 
 ...
 <!-- End HEADER -->
  <!--Start CENTRAL -->
 <div id="central">
  <h1>.:Monitor de Stock - LLANTAS DEPORTIVAS REPLICA:. </h1>
<!--    <table>
        <tr>
            <td>Sucursal:</td>
            <td>
