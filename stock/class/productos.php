...
function getCantidadProductoConFecha($pro_id=0,$suc_id=0) {
    $link=Conectarse();
    $cant_inicial = 0;
    $cantidad_stock = 0;
    $cantidad = 0;
...
function getCantidadProducto($pro_id=0,$suc_id=0,$fecha='') {
    ...
    if ($fecha == ""){
        ...
        $fecha=($fechaNormal->cambiaf_a_mysql($fecha));
    }
    //Recupero cantidad inicial
    $sql="select ifnull(s.stm_cantidad,0) cantidad, s.stm_fecha
        from stock_mensual s
    ...
    if ($pro_controla_stock == "S"){
        ...
        $cantidad=0;
    }//echo("inicial:".$cant_inicial);echo("cant:".$cantidad);
    return $cantidad;
}
...
//Dicha función debe validar el ultimo stock de dicho producto / sucursal dentro de la tabla stock_mensual y
//luego sumar y restar los movimientos de la tabla movimientos_stock.
function PermiteCarga($pro_id=0,$suc_id=0,$fecha='',$pro_cantidad=0) {
    include_once 'fechas.php';
    $link=Conectarse();
    $cant_prod='';
//echo("f:".$fecha);
    $fechaNormal = new fechas();
    $startTime=date('Y-m-d',strtotime($fechaNormal->cambiaf_a_mysql($fecha)));
    $endTime = date('Y-m-d', mktime(0, 0, 0, date('m'),date('d'),date('Y')));//strtotime('2010-05-10' 12:00);
    //echo("st:".$startTime);echo("-et:".$endTime);
    //echo("ststr:".strtotime($startTime));echo("-etstr:".strtotime($endTime)).'</br>';
    
    while (strtotime($startTime) <= strtotime($endTime)) {
		//echo "startTime".$startTime.'</br>';
        $cant_prod=$this->getCantidadProducto($pro_id,$suc_id,$fechaNormal->cambiaf_a_normal($startTime));
        //echo("cant_prod:".$cant_prod);echo("-pro_cantidad:".$pro_cantidad);
...
function getproductosCombo($pro_id=0, $tip_id=0) {
    $link=Conectarse();
    $html = "";
    $sql=" select 'Elige' as pro_descripcion, '0' as pro_id, 0 as orden
        union
