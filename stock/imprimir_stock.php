...
include_once 'class/reportes.php';
        ...
        $pdf->SetFontSize(14);
        $pdf->SetXY(1, 38);
        $pdf->Write(0, "LISTADO DE STOCK ");			
        $pdf->Line(0,42,290,42);	
		$pdf->Write(0," TIPO DE PRODUCTO: ");
		$pdf->Write(0,$_GET["tip_id"]);	
		$pdf->Write(0," SUCURSAL: ");
		$pdf->Write(0,$_GET["suc_id"]);	
		$pdf->Write(0," FECHA: ");
...
//echo'd:'.$fechaNormal->cambiaf_a_mysql($_POST["desde"]).'-h:'.$fechaNormal->cambiaf_a_mysql($_POST["hasta"]);
        $detalle = $rep->get_rep_stock(
         ...
         while($row= mysql_fetch_assoc($detalle)) {
            ...
            $pdf->Write(0, $row['tr_descripcion']);
            $pdf->SetXY(268, $linea2);
            $pdf->Cell(20,0,$row['cantidad_suc'],0,0,"R");
            $linea2 = $linea2 + 5;
        }
