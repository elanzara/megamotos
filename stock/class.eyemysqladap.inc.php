...
class EyeMySQLAdap
{
	...
	{
		/*EDU*/
		/*OJO ACA CON LO QUE LE AGREGUE PORQUE NO VA A FUNCIONAR SIEMPRE - PUEDE VARIAR LA CANTIDAD*/
/*		$pos = strpos($query, "COUNT(*)");
		if ($pos == true){
			//$query = "SELECT COUNT(*) FROM productos p , tipo_productos tp , marcas m , modelos mo WHERE (p.pro_estado = 0 and p.tip_id = tp.tip_id and p.mar_id = m.mar_id and p.mod_id = mo.mod_id) LIMIT 1";
            $query = "SELECT COUNT(*) FROM productos p where p.pro_estado = 0";
		}
*/
		/*FIN DE LO AGREGADO*/
...
//        $query = "";
		...
		if ($result == false)
			trigger_error('Uncovered an error in your SQL query script: "' . $this->error() . '" <br>Query: '.$query);
	...
	public function select($fields, $table, $where = false, $orderby = false, $limit = false)
	{
		...
		/*EDU - MODIFICADO PORQUE FUNCIONA MAL PARA VARIAS TABLAS*/
