...
//                      </a><br>
                        ...
                        <input type="text" size="6" id="filter-value-' . $t . '" value="" tabindex="' . $i . '" >&nbsp;                        
                        </div>';
                        $i = $i + 1;
                        //onkeydown="Navegar_prox_item(this.id);"
...
//                                                and m.suc_id = 3)" . " LIKE '" . $filter_value . "')";
 //                       } else if ($this->filter['Column'] == "cantidad_Taller"){
  ...
  //                                              and m.suc_id = 1)" . " LIKE '" . $filter_value . "')";
                        } else if ($this->filter['Column'] == "cantidad_Caseros"){
                        $filter_query .= "(" . "(select ifnull(sum(cast(m.cantidad as signed)*(case m.tim_id when 1 then 1 else (-1) end)),0) cantidad_Caseros
                                                from movimientos_stock m
                                                where m.estado=0
                                                and m.pro_id = p.pro_id
                                                and m.suc_id = 7)" . " LIKE '" . $filter_value . "')";
                        } else if ($this->filter['Column'] == "cantidad_Forest"){
                        $filter_query .= "(" . "(select ifnull(sum(cast(m.cantidad as signed)*(case m.tim_id when 1 then 1 else (-1) end)),0) cantidad_Forest
                                                from movimientos_stock m
                                                where m.estado=0
                                                and m.pro_id = p.pro_id
                                                and m.suc_id = 5)" . " LIKE '" . $filter_value . "')";
...
//        echo "Asi se arma el query: " . $query ."<br>";
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				{//echo'value:'.$value.'-key:'.$key.'</br>';
                                    ...
                                    if ($key == 'cantidad_Caseros') {//echo'key:'.$key.'</br>';
                                        $pro_cant = new productos();
                                        $value=$pro_cant->getCantidadProducto($value, 7);//echo'v:'.$value.'-k:'.$key.'</br>';
                                    }
                                    if ($key == 'cantidad_Forest') {//echo'key:'.$key.'</br>';
                                        $pro_cant = new productos();
                                        $value=$pro_cant->getCantidadProducto($value, 5);//echo'v:'.$value.'-k:'.$key.'</br>';
                                    }
...
//        alert ('FILTER:' + tblfilter);
		...
		echo "function Navegar_prox_item(id) {alert ('ho1');
                        if (window.event.keyCode == 9){alert ( 'tab'); alert (id);
                        } ;
                    }\n";
